...
void shuffle_playlist() {
  ...
  std::uniform_int_distribution<uint8_t> rand_times(MIN_RAND_ITERATIONS, MAX_RAND_ITERATIONS) ;

  uint8_t nb_of_random = rand_times(gen) ;


  #ifdef INFO
  #if (INFO >= 2)
  fprintf(stdout,"\n%s: Mixing playlist %d times...\n", g_get_prgname(), nb_of_random) ; fflush(stdout) ;
  #endif
  #endif

  for (uint8_t c=0 ; c < nb_of_random ; c++) {

    // Mix shuffle list nb_of_random times.
  ...
  for (int c=0 ; c < (int) playlist_shuffle_indexes.size() ; c++) {
    ...
    fprintf(stdout,"\n%s: Shuffle Playlist song N° %03d\n -> %s\n", g_get_prgname(), c+1, g_path_get_basename(folder_music_playlist_container.at(playlist_shuffle_indexes.at(c)).c_str()) ) ;
