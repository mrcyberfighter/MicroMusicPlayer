...
void play_music(GtkWidget *widget, gpointer user_data) {
  ...
  if (! play_folder_content && ! is_playing) {
    ...
    if( ! libvlc_media_player_is_playing(media_player) ) {
      ...
      if (is_stopped) {
        ...
        if ( cover_image != settings.path_to_default_image ) {  // Cover image not equal to default image.
          ...
          GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file(image_cover_frame.c_str(), NULL);
        ...
        gtk_button_set_label(GTK_BUTTON(gui->folder_chooser_button), folder_basename.c_str() ) ;
...
void prev_music(GtkWidget *widget, gpointer user_data)  {
  ...
  if (! play_folder_content) { return ; }  // Play a single file so we don't need this functionality.
...
void change_volume(GtkScaleButton *button, gdouble value, gpointer user_data) {
  ...
  libvlc_audio_set_volume(media_player, value * 100.0) ;
