...
void row_selection_avoiding(GtkListBox *listbox, GtkListBoxRow *row, gpointer user_data) {
...
void update_playlist() {
  ...
  for (int c = 0 ; c < (int) folder_music_playlist_container.size() ; c++) {
    ...
    char number_fmt_str[8] = {0} ;
    sprintf(number_fmt_str, "%3d", c+1 ) ;
...
void build_playlist(GtkWidget *widget, gpointer user_data) {
  ...
  gtk_window_set_icon_from_file(GTK_WINDOW(gui->playlist->playlist_displaying_dialog), PATH_TO_PRG_IMAGE, NULL) ;
