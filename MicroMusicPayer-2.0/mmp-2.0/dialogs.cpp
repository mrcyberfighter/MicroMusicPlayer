...
void display_message_dialog(const char *title, const char *msg, GtkMessageType type, GtkButtonsType button_type) {
  ...
  gtk_window_set_icon_from_file(GTK_WINDOW(message_dialog), PATH_TO_PRG_IMAGE, NULL) ;
...
void display_about_dialog(GtkWidget *widget, gpointer *user_data) {
  ...
  gtk_about_dialog_set_program_name(GTK_ABOUT_DIALOG(about_dialog), "Micro Music Player") ;

  gtk_about_dialog_set_version(GTK_ABOUT_DIALOG(about_dialog), "2.0") ;

  gtk_about_dialog_set_copyright(GTK_ABOUT_DIALOG(about_dialog), "This program is under copyright from the GPLv3 license." ) ;
  ...
  gtk_about_dialog_set_website(GTK_ABOUT_DIALOG(about_dialog), "http://www.open-source-projects.net/Micro_Music_Player/Micro_Music_Player_presentation.html") ;

  gtk_about_dialog_set_website_label(GTK_ABOUT_DIALOG(about_dialog), "Micro Music Player Website" ) ;
  ...
  const char *programmer[] = { "Eddie Brüggemann <mrcyberfighter@gmail.com>", NULL } ;

  gtk_about_dialog_set_authors(GTK_ABOUT_DIALOG(about_dialog),  programmer) ;

  const char *artist[] = { "Eddie Brüggemann <mrcyberfighter@gmail.com>", NULL } ;

  gtk_about_dialog_set_artists( GTK_ABOUT_DIALOG(about_dialog), artist) ;

  const char *documenters[] = { "Eddie Brüggemann <mrcyberfighter@gmail.com>", NULL } ;
  ...
  gtk_window_set_icon_from_file(GTK_WINDOW(about_dialog), PATH_TO_PRG_IMAGE, NULL) ;
...
void get_folder_to_play(GtkWidget *widget, gpointer user_data) {
  ...
  gchar *folderpath ;  // Selected folder.
  ...
  switch (result) {
      ...
      case GTK_RESPONSE_ACCEPT :
        ...
        if (! g_file_test(folderpath, G_FILE_TEST_IS_DIR) ) {
  ...
  current_folder = g_path_get_dirname(folderpath) ;
...
void get_file_to_play(GtkWidget *widget, gpointer user_data) {
  ...
  GtkWidget *file_chooser_dialog = gtk_file_chooser_dialog_new("Choose a folder contains music files to listen.", NULL, GTK_FILE_CHOOSER_ACTION_OPEN, "Cancel", GTK_RESPONSE_CANCEL, "Open", GTK_RESPONSE_ACCEPT, NULL);
  ...
  gtk_file_chooser_set_filter(GTK_FILE_CHOOSER(file_chooser_dialog), GTK_FILE_FILTER(file_filter)) ;
  ...
  switch (result) {
      ...
      case GTK_RESPONSE_ACCEPT :
        ...
        filepath = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(file_chooser_dialog));
        gtk_widget_destroy(file_chooser_dialog);
        ...
        if (! g_file_test(filepath, G_FILE_TEST_IS_REGULAR) ) {
          ...
          #ifdef DEBUG
          fprintf(stdout,"%s(...) %s is not a file !\n", __func__, filepath) ;
          #endif
  ...
  current_folder = g_path_get_dirname(filepath) ;
...
void configure_program_dialog(GtkWidget *widget, gpointer user_data) {
  ...
  gtk_window_set_icon_from_file(GTK_WINDOW(configure_dialog), PATH_TO_PRG_IMAGE, NULL) ;
  ...
  gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(folder_selecting_button), settings.path_to_music_folder.c_str()) ;
