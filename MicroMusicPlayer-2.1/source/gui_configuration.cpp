...
static void reconfigure_interface_icons() {
...
void reconfigure_interface_size(GtkWidget *widget, int *user_data) {
  ...
  if (default_image) {
    ...
    auto new_size = resizer.resizer(imaging_cover_container.at(music_song_counter), settings.image_resized_size, settings.image_resized_size, false) ;
  
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file(imaging_cover_container.at(music_song_counter).c_str(), &error);
...
void set_default_folder(GtkFileChooserButton *chooser, gpointer user_data) {
  ...
  #ifdef INFO
  #if (INFO >= 1)
  fprintf(stdout,"%s(...) change music default folder to:\n%s\n", __func__, filepath ) ;
  fflush(stdout) ;
  #endif
  #endif
  ...
  current_folder = settings.path_to_music_folder = filepath ;
